...
async def notifications():
  ...
  if(gerwant.isOnline() == False):
    ...
    data = gerwant.getStreamData()["data"][0]
    await channel.send("@everyone Właśnie odpaliłem maśniutkiego streama, wbijaj wariacie! 🐺🐺🐺")
    
    embed = discord.Embed(title=data["title"], url="https://twitch.tv/gerw4nt", description="Stream online 🐺🐺🐺", color=0xb50c0f, timestamp=datetime.datetime.utcnow())
    ...
    embed.add_field(name="Gra: ", value=data["game_name"], inline = True)
